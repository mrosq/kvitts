...
CRUD mot Supabase `expenses`-tabellen via `supabase.js` (hamtaUtgifter, laggTillUtgiftRum, uppdateraUtgift, raderaUtgiftRum) med mappning DB ↔ klient. Write-paths i app.js (laggTillUtgift, sparaEdit, raderaUtgift) routar till Supabase vid rum-session, localStorage annars. Polling var 7s via setInterval, pausas vid document.hidden, stoppas vid doljAllaSkärmar/unload. Header i rum-läge visar projektnamn + antal personer; klickbar → deltagar-modal med chips. 67/67 tester gröna.
